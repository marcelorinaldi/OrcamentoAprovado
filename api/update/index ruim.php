...
try {
    ...
    $sql = "UPDATE `produto` SET `nome` = 'igor' WHERE `id` = 1 limit 1";
	//UPDATE `produto` SET  = '11', `valor` = '22', `imagem` = '33', `quantidade` = '44', `apagado` = '0' WHERE `produto`.`id` = 32;
...
$conn->close();
	...
	$response = array('success' => true, 'message' => 'Nok_111');
        echo json_encode($response);
}
		
	}
}else{
	$response = array('success' => true, 'message' => 'Nok_222');
        echo json_encode($response);
}
